...
const DAILY_FEED = [
  ...
  { name: "이은혜", avatar: "이", time: "오늘 오전 7:58", text: "새벽 기도 후 아침 노을이 너무 아름다웠어요 ☀️" },
      ...
      <div className="relative rounded-xl sm:rounded-2xl overflow-hidden" style={{ minHeight: 120 }}>
        ...
        <div className="absolute inset-0 z-10 px-4 sm:px-6 py-4 sm:py-0 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-green-200 text-xs mb-1 flex items-center gap-1">
            ...
            <p className="text-green-100 text-xs sm:text-sm hidden sm:block">행복과 영원으로 초대하는 일광교회 커뮤니티입니다</p>
          </div>
          <div className="bg-white/15 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 text-sm max-w-[200px] sm:max-w-xs border border-white/20 flex flex-col justify-center self-center shrink-0 hidden sm:flex">
            ...
            <p className="text-white font-medium leading-relaxed text-xs sm:text-sm">"{TODAY_VERSE.text}"</p>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-2 sm:gap-3">
          {COMMUNITY_MENUS.map((q) => (
            ...
            >
              ...
              <span className="text-[11px] sm:text-xs font-semibold text-gray-700 group-hover:text-[#2E7D32] leading-tight">{q.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-50">
            ...
            <Link href="/dashboard/daily" className="flex items-center gap-1 text-xs text-gray-400 hover:text-[#2E7D32] hover:underline">
          ...
          <div className="divide-y divide-gray-50 flex-1">
            {DAILY_FEED.map((f, i) => (
              <div key={i} className="px-4 sm:px-5 py-3 flex items-start gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 bg-gray-100 text-gray-600">
                  {f.avatar}
                </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-50">
            ...
            <Link href="/dashboard/events" className="flex items-center gap-1 text-xs text-gray-400 hover:text-[#2E7D32] hover:underline">
          ...
          <div className="divide-y divide-gray-50 flex-1">
            {UPCOMING_EVENTS.map((e, i) => (
              <div key={i} className="px-4 sm:px-5 py-3 flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm font-medium text-gray-800 truncate">{e.title}</p>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full border bg-gray-50 text-gray-500 border-gray-200">{e.category}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-50">
            ...
            <Link href="/dashboard/blog" className="flex items-center gap-1 text-xs text-gray-400 hover:text-[#2E7D32] hover:underline">
              전체보기 <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="divide-y divide-gray-50 flex-1">
            {BLOG_POSTS.map((p, i) => (
              <div key={i} className="px-4 sm:px-5 py-3 sm:py-4 hover:bg-gray-50 transition-colors cursor-pointer group">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-gray-100 text-gray-600">{p.dept}</span>
                  <span className="text-[10px] text-gray-400">{p.date}</span>
                </div>
                <p className="text-xs sm:text-sm font-medium text-gray-800 group-hover:text-[#2E7D32] transition-colors leading-snug">{p.title}</p>
              </div>
            ))}
          </div>
          <div className="px-4 sm:px-5 py-3 border-t border-gray-50">
        ...
        <div className="relative rounded-xl sm:rounded-2xl overflow-hidden min-h-[160px]">
          ...
          <div className="relative z-10 p-4 sm:p-6">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-white text-[#2E7D32] rounded-lg text-xs sm:text-sm font-bold hover:bg-green-50 transition-colors"
            >
          ...
          <div className="relative z-10 p-4 sm:p-6">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4">
              {["새가족교육", "제자훈련", "청년성경공부", "소그룹리더"].map(t => (
                <span key={t} className="text-xs px-2 py-0.5 bg-white/20 text-white rounded-full">{t}</span>
              ))}
            </div>
            <Link
              href="/dashboard/education"
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-white text-[#1a237e] rounded-lg text-xs sm:text-sm font-bold hover:bg-blue-50 transition-colors"
            >
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
              <Link href="/dashboard/members" className="flex items-center gap-2.5 sm:gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] sm:text-xs text-gray-500 truncate">회원 관리</p>
              ...
              <Link href="/dashboard/contacts" className="flex items-center gap-2.5 sm:gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] sm:text-xs text-gray-500 truncate">미확인 문의</p>
              ...
              <Link href="/dashboard/notices/announcements" className="flex items-center gap-2.5 sm:gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] sm:text-xs text-gray-500 truncate">최근 공지</p>
                  <p className="text-sm font-bold text-gray-900">3건</p>
                </div>
              </Link>
              <Link href="/dashboard/dept-share" className="flex items-center gap-2.5 sm:gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] sm:text-xs text-gray-500 truncate">부서나눔</p>
