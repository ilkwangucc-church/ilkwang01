...
export default function EditAnnouncementPage() {
  ...
  useEffect(() => {
    fetch("/api/notices")
      .then((r) => r.json())
      .then((data: { id: number; title: string; category: string; content: string; pinned: boolean; published: boolean }[]) => {
        const notice = data.find((n) => String(n.id) === String(id));
        if (notice) {
          setForm({
            title:     notice.title,
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">공지안내 수정</h1>
      </div>
